...
func (c *Client) createConnection(info *ConnectionInfo) (*ssh.Client, error) {
	// 调试信息输出到stderr，避免干扰MCP协议通信
	log.Printf("[SSH Debug] 开始创建SSH连接到 %s:%d，用户: %s", info.Host, info.Port, info.User)
	...
	if c.config.KnownHostsFile != "" {
		log.Printf("[SSH Debug] 使用known_hosts文件: %s", c.config.KnownHostsFile)
		hostKeyCallback, err := knownhosts.New(c.config.KnownHostsFile)
		if err != nil {
			log.Printf("[SSH Debug] 加载known_hosts文件失败: %v", err)
			return nil, fmt.Errorf("加载known_hosts文件失败: %w", err)
		}
		sshConfig.HostKeyCallback = hostKeyCallback
	} else {
		// 警告：跳过主机密钥验证（仅用于开发环境）
		log.Printf("[SSH Debug] 警告: 跳过主机密钥验证（不安全，仅用于开发环境）")
		sshConfig.HostKeyCallback = ssh.InsecureIgnoreHostKey()
	}

	// 设置认证方法
	log.Printf("[SSH Debug] 准备认证方法...")
	auth, err := c.getAuthMethods(info)
	if err != nil {
		log.Printf("[SSH Debug] 获取认证方法失败: %v", err)
	...
	log.Printf("[SSH Debug] 正在连接 %s，超时: %v", address, c.config.Timeout)

	client, err := ssh.Dial("tcp", address, sshConfig)
	if err != nil {
		log.Printf("[SSH Debug] SSH连接失败: %v", err)
		return nil, fmt.Errorf("连接SSH服务器失败: %w", err)
	}

	log.Printf("[SSH Debug] SSH连接成功建立")
	return client, nil
}
...
func (c *Client) getAuthMethods(info *ConnectionInfo) ([]ssh.AuthMethod, error) {
	...
	log.Printf("[SSH Debug] 开始准备认证方法，主机: %s:%d, 用户: %s", info.Host, info.Port, info.User)

	// 如果提供了密码，优先使用密码认证
	if info.Password != "" {
		log.Printf("[SSH Debug] 添加密码认证方法 (密码长度: %d)", len(info.Password))
		authMethods = append(authMethods, ssh.Password(info.Password))
	} else {
		log.Printf("[SSH Debug] 未提供密码，跳过密码认证")
	}
	...
	if keyFile != "" {
		log.Printf("[SSH Debug] 尝试使用私钥文件: %s", keyFile)
		// 扩展路径
		if keyFile[0] == '~' {
			homeDir, err := os.UserHomeDir()
			if err != nil {
				log.Printf("[SSH Debug] 获取用户主目录失败: %v", err)
		...
		if err != nil {
			log.Printf("[SSH Debug] 读取私钥文件失败: %v", err)
		...
		if err != nil {
			log.Printf("[SSH Debug] 解析私钥失败: %v", err)
			return nil, fmt.Errorf("解析私钥失败: %w", err)
		}

		log.Printf("[SSH Debug] 成功添加私钥认证方法")
		authMethods = append(authMethods, ssh.PublicKeys(signer))
	} else {
		log.Printf("[SSH Debug] 未指定私钥文件，跳过私钥认证")
	}

	// 最后添加SSH Agent认证（如果可用）
	if agentConn, err := net.Dial("unix", os.Getenv("SSH_AUTH_SOCK")); err == nil {
		log.Printf("[SSH Debug] SSH Agent 可用，添加 Agent 认证方法")
		...
		log.Printf("[SSH Debug] SSH Agent 不可用: %v", err)
	}

	if len(authMethods) == 0 {
		log.Printf("[SSH Debug] 错误: 没有可用的认证方法")
		return nil, fmt.Errorf("没有可用的认证方法")
	}

	log.Printf("[SSH Debug] 共准备了 %d 个认证方法", len(authMethods))
